...
- (void)galleryMedia:(FGalleryMedia *)media didLoadFullsizeVideo:(NSURL *)videoUrl
{
	...
	if( _currentIndex == media.tag )
	{
        ...
        if(!videoView.video) // Not loaded yet
        {        
            AVURLAsset * asset = [AVURLAsset URLAssetWithURL:videoUrl options:nil];
            
        
            
            videoView.video = [[GPUImageMovie alloc] initWithAsset:asset];
        
            videoView.video.playAtActualSpeed = YES;
            videoView.video.playSound = YES;
            videoView.filter = [[GPUImageSharpenFilter alloc] init];
            [videoView.video addTarget:videoView.filter];
            [videoView.filter forceProcessingAtSize:[videoView.view sizeInPixels]];
            [videoView.filter setSharpness:2.0f];
            
            [videoView.filter addTarget:videoView.view];
